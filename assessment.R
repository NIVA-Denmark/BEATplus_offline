...
#===============================================================================
# function Assessment
Assessment<- function(assessmentdata,summarylevel=NA){
  
  summarylevel=ifelse(is.na(summarylevel),-99,summarylevel)
  ...
  if(n<nreq){
    ...
    groups$SpatialAssessmentUnit <- as.character(groups$SpatialAssessmentUnit)
    ...
    if(summarylevel==0){
      ...
      # no summary level specified
      # return all as a list
      
      res <- list(Errors,Indicators,QEspr,dataSpecies,dataSpeciesGroup,QEdata,OverallQE)
      return(res)
    }
    #
  }
